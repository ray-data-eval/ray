...
class OpState:
    ...
    def add_output(self, ref: RefBundle) -> None:
        ...
        Note: `partition_index` is also stamped at task submission time by
        MapOperator._submit_data_task (see DataOpTask.partition_index), which
        is robust to out-of-order task completion. The propagation below is
        what supplies `creation_time`, which the LLF policy needs and which
        submission-time stamping does not provide.
        """
        ...
        if self._llf_input_metadata and ref.creation_time is None:
            ...
            if ref.partition_index is None:
                ref.partition_index = partition_index
            # One metadata entry is consumed per output bundle.
    ...
    def dispatch_next_task(self) -> None:
        ...
        for i, inqueue in enumerate(self.inqueues):
            ...
            if ref is not None:
                ...
                # Separately, track (creation_time, partition_index) so the
                # LLF policy can stamp the outputs this input produces.
...
def select_operator_to_run(
    ...
    if ops:
        # Three-way policy dispatch. The two emulation policies are mutually
        # exclusive with each other and with the default; `scheduling_policy`
        # selects exactly one.
        #   "microbatch"            Spark-Streaming / Drizzle BSP emulation
        #   "llf_v1"/"llf_v2"/"edf" Cameo (NSDI'21) deadline-driven emulation
        #   None                    Radar's own policy (paper Algorithm 1)
        if ctx.scheduling_policy == "microbatch":
            ...
            # Radar's policy (Algorithm 1, line 16): run metadata-only operators
            # first, then choose the operator with the least buffered output.
            selected_op = min(
                ops,
                key=lambda op: (
        ...
        # _microbatch_select may legitimately return None when a stage
        # barrier blocks every candidate, so this guard is required.
